...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        while (!input.equals("q")) {
            System.out.println(ANSI_YELLOW + "Введите имя и фамилию, или q для остановки ввода: ");
            data2 = new ArrayList<>();
            input = sc.nextLine();
            if (!input.equals("q")) {
                names = input.split(" ");
        ...
        System.out.println(ANSI_YELLOW + "Повторяющиеся имена:");
        ...
        System.out.println(ANSI_YELLOW + "Сортировка по популярности имён: ");
        hashMapSpecialPrint(orderIndexes, data);

        orderIndexes = getDefaultOrderIndexes(data);

        orderIndexes = getSortedArrayIndexes(data, 2, orderIndexes, true);
        System.out.println(ANSI_YELLOW + "Сортировка по популярности номеров:");
        hashMapSpecialPrint(orderIndexes, data);
        ...
        for (int k : orderIndexes) {
            for (String[] array : data.get(k)) {
                for (String element : array) {
                    System.out.print(ANSI_RESET + element + " ");
                }
            }
            System.out.println();
        }
    ...
    static void namesCountPrint(HashMap<String, Integer> hashMap, String[] keysOrder) {
        for (String i : keysOrder) {
            System.out.println(ANSI_RESET + i + ":" + hashMap.get(i));
        }
    }
